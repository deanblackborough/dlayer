...
/**
 * Button content item tool
 *
 * @author Dean Blackborough <dean@g3d-development.com>
 * @copyright G3D Development Limited
 * @license https://github.com/Dlayer/dlayer/blob/master/LICENSE
 */
class Dlayer_DesignerTool_ContentManager_Button_Tool extends Dlayer_Tool_Content
{
    /**
    ...
    protected function paramsExist(array $params)
    {
        ...
        if (array_key_exists('name', $params) === true &&
            array_key_exists('label', $params) === true
        ) {
    ...
    {
        $data_added = false;
        $content_id = $this->addContentItem('Button');

        if ($content_id !== false) {
            $model = new Dlayer_DesignerTool_ContentManager_Button_Model();
            $data_added = $model->add($this->site_id, $this->page_id, $content_id, $this->params);
        }

        if ($content_id !== false && $data_added === true) {
            Dlayer_Helper::sendToInfoLog('Button added to site id: ' . $this->site_id . ' page id: ' . $this->page_id .
    ...
    protected function validateInstances($site_id, $content_id)
    {
        ...
        $instances = $model->instancesOfData($site_id, $content_id, 'Button');
