...
/**
 * Form for the button content item
 *
 * @author Dean Blackborough
 * @copyright G3D Development Limited
 * @license https://github.com/Dlayer/dlayer/blob/master/LICENSE
 */
class Dlayer_DesignerTool_ContentManager_Button_Form extends Dlayer_Form_Tool_Content
{
    /**
    ...
    public function init()
    {
        ...
        $this->addElementsToForm('button_content_item', 'Button', $this->elements);
    ...
    protected function generateUserElements()
    {
        ...
        $name->setLabel('Name');
        $name->setAttribs(array('size'=>50, 'maxlength'=>255, 'placeholder'=>'e.g., Link to GitHub project',
        ...
        $content = new Zend_Form_Element_Text('label');
        $content->setLabel('Label');
        $content->setAttribs(array('size'=>50, 'maxlength'=>255, 'placeholder'=>'e.g., Download',
            'class'=>'form-control input-sm'));
        $content->setDescription('Enter the label for the button.');
        $content->setBelongsTo('params');
        $content->setRequired();

        if(array_key_exists('label', $this->data) === TRUE && $this->data['label'] !== FALSE)
        {
            $content->setValue($this->data['label']);
        }

        $this->elements['label'] = $content;
    }
}
